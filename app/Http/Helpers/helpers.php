...
function menuActive($routeName, $type = null, $param = null) {
    $class = $type == 3 ? 'side-menu--open' : ($type == 2 ? 'sidebar-submenu__open' : 'active');
    ...
    } else if ($routeName && request()->routeIs($routeName)) {
        if ($param) {
...
function showEmailAddress($email) {
    if (!is_string($email) || $email === null || strpos($email, '@') === false) {
        return '';
    }
