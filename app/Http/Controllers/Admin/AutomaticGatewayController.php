...
class AutomaticGatewayController extends Controller
{
    ...
    // AJAX: Add new supported currency
    public function addCurrency(Request $request)
    {
        $request->validate([
            'gateway_code' => 'required|exists:gateways,code',
            'currency_code' => 'required|string|max:10',
            'currency_symbol' => 'required|string|max:10',
        ]);

        $gateway = \App\Models\Gateway::where('code', $request->gateway_code)->firstOrFail();
        $supported = (array) $gateway->supported_currencies;
        $supported[$request->currency_code] = $request->currency_symbol;
        $gateway->supported_currencies = $supported;
        $gateway->save();
