            ...
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.frontend.sections.content', $key) }}" class="disableSubmission" method="POST" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="{{ $key }}" value="1"> <!-- Ensure section key is sent -->
                        @if (@$data)
                        ...
                        <div class="row">
                            ...
                            @foreach ($section->element as $k => $content)
                                    ...
                                    @foreach ($content as $imgKey => $image)
                                        <div class="col-md-4">
                                            ...
                                            <div class="form-group">
                                                <label for="image-upload-input{{ $loop->index }}">{{ __(keyToTitle($imgKey)) }}</label>
                                ...
                                @elseif($content == 'icon')
                                    <div class="form-group">
                                        <label for="{{ $inputId }}">{{ keyToTitle($k) }}</label>
                                        <div class="input-group">
                                            <input type="text" id="{{ $inputId }}" class="form-control iconPicker icon" autocomplete="off" name="{{ $k }}" value="{{ old($k,@$data->data_values->$k) }}" required>
                                ...
                                @else
                                    @if ($content == 'textarea')
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label for="{{ $inputId }}">{{ __(keyToTitle($k)) }}</label>
                                                <textarea id="{{ $inputId }}" rows="10" class="form-control" name="{{ $k }}" required>{{ old($k,@$data->data_values->$k) }}</textarea>
                                            </div>
                                        </div>
                                    @elseif($content == 'textarea-nic')
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label for="{{ $inputId }}">{{ __(keyToTitle($k)) }}</label>
                                                <textarea id="{{ $inputId }}" rows="10" class="form-control nicEdit" name="{{ $k }}">{{ old($k,@$data->data_values->$k) }}</textarea>
                                            </div>
                                        </div>
                                    @elseif($k == 'select')
                                        @php
                                            $selectName = $content->name;
                                            $selectId = 'input_' . $selectName;
                                        @endphp
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label for="{{ $selectId }}">{{ __(keyToTitle(@$selectName)) }}</label>
                                                <select id="{{ $selectId }}" class="form-control select2" data-minimum-results-for-search="-1" name="{{ @$selectName }}" required>
                                    ...
                                    @elseif($k == 'slug')
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="{{ $inputId }}" class="form-control" name="slug" value="{{ old($k,@$data->slug) }}" required>
                                            </div>
                                        </div>
                                    @else
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="{{ $inputId }}" class="form-control" name="{{ $k }}" value="{{ old($k,@$data->data_values->$k) }}" required>
                                            </div>
                                        </div>
                                    @endif
                                @endif
